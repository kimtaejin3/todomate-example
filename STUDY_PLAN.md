...
- GoalContext / TodoContext를 도메인별로 분리
  - Graph 컴포넌트는 goals의 color만 필요한데, todo가 변경될 때마다 리렌더링되는 문제를 직접 경험
  - 분리 후 todo 토글 시 Graph가 리렌더링되지 않는 것을 확인
- **관찰 포인트**
  - Context를 쪼개지 않으면 dispatch만 쓰는 컴포넌트도 state 변경 시 리렌더링됨
  - Graph처럼 goals만 참조하는 컴포넌트가 todos 변경에도 리렌더링되는 현상 → 도메인별 Context 분리 필요성 체감
